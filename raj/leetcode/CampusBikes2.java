...
public class CampusBikes2 {
	...
	// https://leetcode.com/problems/campus-bikes-ii/discuss/303375/Java-straightforward-DFS-solution
	...
	void dfs(int[][] workers, int i, int[][] bikes, boolean[] used, int sum) {
		...
		for (int j = 0; j < bikes.length; ++j) {
			if (used[j])
				continue;
			used[j] = true;
			dfs(workers, i + 1, bikes, used, sum + manhattanDistance(workers[i], bikes[j]));
			used[j] = false;
		}
	}
