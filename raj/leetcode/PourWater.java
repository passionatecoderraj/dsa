...
public class PourWater {
	...
	// https://leetcode.com/problems/pour-water/discuss/113002/Share-my-Java-Solution
	// Time : O(n*V), Space : O(1)
	public int[] pourWater(int[] a, int V, int K) {
		if (a == null || a.length == 0 || V == 0) {
			return a;
		}
		while (V-- > 0) {
			int index = K;
	...
	// Time : O(n*V), Space : O(1)
	public int[] pourWater2(int[] a, int V, int K) {
		if (a == null || a.length == 0 || V == 0) {
			return a;
		}
		while (V-- > 0) {
	...
	public static void main(String[] args) {
		...
		res = obj.pourWater(heights, 4, 3);
		System.out.println(Arrays.toString(res));

		res = obj.pourWater(new int[] { 3, 1, 3 }, 5, 1);
		System.out.println(Arrays.toString(res));
