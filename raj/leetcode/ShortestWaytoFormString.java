package com.raj.leetcode;

/**
 * 
 * @author Raj
 *
 *From any string, we can form a subsequence of that string by deleting some number of characters (possibly no deletions).

Given two strings source and target, return the minimum number of subsequences of source such that their concatenation equals target. If the task is impossible, return -1.

 

Example 1:

Input: source = "abc", target = "abcbc"
Output: 2
Explanation: The target "abcbc" can be formed by "abc" and "bc", which are subsequences of source "abc".
Example 2:

Input: source = "abc", target = "acdbc"
Output: -1
Explanation: The target string cannot be constructed from the subsequences of source string due to the character "d" in target string.
Example 3:

Input: source = "xyz", target = "xzyxz"
Output: 3
Explanation: The target string can be constructed as follows "xz" + "y" + "xz".
 

Constraints:

Both the source and target strings consist of only lowercase English letters from "a"-"z".
The lengths of source and target string are between 1 and 1000.

 */
public class ShortestWaytoFormString {

	/*
	 * ti: index for target string si: index for source string
	 */
	//https://leetcode.com/problems/shortest-way-to-form-string/discuss/304193/Java-Two-Pointers-Solution-With-Explanation
	
	public int shortestWay(String s, String t) {
		int tI = 0;
		int ans = 0;
		while (tI < t.length()) {
			int pt = tI;
			for (int sI = 0; sI < s.length(); sI++) {
				if (tI < t.length() && s.charAt(sI) == t.charAt(tI)) {
					tI++;
				}
			}

			if (tI == pt) {
				return -1;
			}
			ans++;
		}

		return ans;
	}

	// Time :O(n), Space : O(1)
	public int shortestWay2(String s, String t) {
		int i = 0, n = s.length(), j = 0;
		while (j < t.length()) {
			int c = 0;
			while (s.charAt(i % n) != t.charAt(j)) {
				i++;
				if (++c == n)
					return -1;
			}
			i++;
			j++;
		}
		return (i / n) + (i % n == 0 ? 0 : 1);
	}

	public static void main(String args[]) {
		ShortestWaytoFormString obj = new ShortestWaytoFormString();
		int res = -1;
		res = obj.shortestWay("abc", "abcbc");
		System.out.println(res);

		res = obj.shortestWay("abc", "acdbc");
		System.out.println(res);

		res = obj.shortestWay("xyz", "xzyxz");
		System.out.println(res);
	}

}
