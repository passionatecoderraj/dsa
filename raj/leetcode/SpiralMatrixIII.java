package com.raj.leetcode;

import java.util.Arrays;

/**
 * 
 * @author Raj
 * 
 * On a 2 dimensional grid with R rows and C columns, we start at (r0, c0) facing east.

Here, the north-west corner of the grid is at the first row and column, and the south-east corner of the grid is at the last row and column.

Now, we walk in a clockwise spiral shape to visit every position in this grid. 

Whenever we would move outside the boundary of the grid, we continue our walk outside the grid (but may return to the grid boundary later.) 

Eventually, we reach all R * C spaces of the grid.

Return a list of coordinates representing the positions of the grid in the order they were visited.

 

Example 1:

Input: R = 1, C = 4, r0 = 0, c0 = 0
Output: [[0,0],[0,1],[0,2],[0,3]]


 

Example 2:

Input: R = 5, C = 6, r0 = 1, c0 = 4
Output: [[1,4],[1,5],[2,5],[2,4],[2,3],[1,3],[0,3],[0,4],[0,5],[3,5],[3,4],[3,3],[3,2],[2,2],[1,2],[0,2],[4,5],[4,4],[4,3],[4,2],[4,1],[3,1],[2,1],[1,1],[0,1],[4,0],[3,0],[2,0],[1,0],[0,0]]


 

Note:

1 <= R <= 100
1 <= C <= 100
0 <= r0 < R
0 <= c0 < C

 */
public class SpiralMatrixIII {

	// https://leetcode.com/problems/spiral-matrix-iii/discuss/158977/Java-15-lines-concise-solution-with-comments
	public int[][] spiralMatrixIII(int R, int C, int r0, int c0) {
		int total = R * C;
		int[][] moves = { { 0, 1 }, { 1, 0 }, { 0, -1 }, { -1, 0 } }; // east, south, west, north
		int direction = 0;
		int[][] res = new int[total][2];
		int len = 0;
		int i = 0;
		res[i++] = new int[] { r0, c0 };
		while (i < total) {
			if (direction == 0 || direction == 2) // when move east or west, the length of path need plus 1
				len++;
			for (int k = 0; k < len; k++) {
				r0 += moves[direction][0];
				c0 += moves[direction][1];
				if (isWithInMatrix(r0, c0, R, C)) {
					res[i++] = new int[] { r0, c0 };
				}
			}
			direction = (direction + 1) % moves.length;
		}
		return res;
	}

	// https://leetcode.com/problems/spiral-matrix-iii/discuss/158970/C++JavaPython-112233-Steps
	public int[][] spiralMatrixIII2(int R, int C, int r0, int c0) {
		int total = R * C;
		int[][] moves = { { 0, 1 }, { 1, 0 }, { 0, -1 }, { -1, 0 } }; // east, south, west, north
		int direction = 0;
		int[][] res = new int[total][2];
		int prev = 0, cur = 1;
		int i = 0;
		res[i++] = new int[] { r0, c0 };
		while (i < total) {
			for (int k = 0; k < cur; k++) {
				r0 += moves[direction][0];
				c0 += moves[direction][1];
				if (isWithInMatrix(r0, c0, R, C)) {
					res[i++] = new int[] { r0, c0 };
				}
			}
			if (prev == cur) {
				cur++;
			} else {
				prev = cur;
			}
			direction = (direction + 1) % moves.length;
		}
		return res;
	}

	public boolean isWithInMatrix(int i, int j, int R, int C) {
		return i >= 0 && i < R && j >= 0 && j < C;
	}

	public static void main(String args[]) throws Exception {

		int R = 1, C = 4, r0 = 0, c0 = 0;
		SpiralMatrixIII obj = new SpiralMatrixIII();
		int[][] res = null;
		res = obj.spiralMatrixIII(R, C, r0, c0);
		for (int[] r : res) {
			System.out.print(Arrays.toString(r) + " ");
		}
		System.out.println();

		res = obj.spiralMatrixIII(5, 6, 1, 4);
		for (int[] r : res) {
			System.out.print(Arrays.toString(r) + " ");
		}
		System.out.println();
	}

}
