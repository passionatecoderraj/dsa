 ...
 *         choosing from digits 1, 9, 3, 4, is 19:39, which occurs 5 minutes
 *         later. It is not 19:33, because this occurs 23 hours and 59 minutes
 *         later. Example 2:
 * 
 ...
 *         returned time is next day's time since it is smaller than the input
 *         time numerically.
 * 
 */
public class NextClosestTime {
	...
	/*
	 * if you have time 12:00(noon) : 720, 12:05 means 725 i.e. (725 - 720) % (24 *
	 * 60) = 5 secs
	 *
	 * 00:10 means (10 - 720) % (24 * 60) = -710 nothing but -> 24*60-710 = 730;
	 *
	 * (if result is negative add to 24*60 because max time we get next closest is
	 * same time which is 24 hours away
	 */
	...
	public String nextClosestTime2(String time) {
		...
		for (int i = 1; i < 1440; i++) {
			...
			for (int j = 0; j < digits.length && allDigitsFound; j++)
				if (!set.contains(digits[j])) {
					allDigitsFound = false;
				}
			if (allDigitsFound) {
				res = digits[0] + "" + digits[1] + ":" + digits[2] + "" + digits[3];
				return res;
			}
