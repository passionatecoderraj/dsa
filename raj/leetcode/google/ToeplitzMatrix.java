 ...
 *         elements are the same, so the answer is True. Example 2:
 * 
 *         Input: matrix = [ [1,2], [2,2] ] Output: False Explanation: The
 *         diagonal "[1, 2]" has different elements.
 * 
 *         Note:
 * 
 *         matrix will be a 2D array of integers. matrix will have a number of
 *         rows and columns in range [1, 20]. matrix[i][j] will be integers in
 *         range [0, 99].
 * 
 *         Follow up:
 * 
 ...
 *         at once? What if the matrix is so large that you can only load up a
...
public class ToeplitzMatrix {
	...
	public static void main(String[] args) {
		...
		result = obj.isToeplitzMatrix(matrix);		
		System.out.println(result);
		int[][] matrix2 = {{1,2},{2,2}};
		result = obj.isToeplitzMatrix(matrix2);		
		System.out.println(result);
