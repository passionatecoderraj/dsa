...
public class PopulateNextRightPointer2 {
    ...
    public void updateNextSiblingInConstantSpace(BinaryTreeNodeWithSibling<Integer> root) {
        BinaryTreeNodeWithSibling<Integer> next_level_start = null; // head of the next level
        BinaryTreeNodeWithSibling<Integer> cur = root; // current node of current level
        BinaryTreeNodeWithSibling<Integer> prev = null;// the leading node on the next level
        while (cur != null) {
            while (cur != null) {
                // left child
